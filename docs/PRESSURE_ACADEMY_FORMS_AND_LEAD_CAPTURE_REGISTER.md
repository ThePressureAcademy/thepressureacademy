...
| `/shop/apparel/` | Drop 01 curiosity and first-access list: required name/email plus capsule choice (`capsule`: core/women/both), optional public-signal preference (`first_choice`), `size_band`, and `context`, hidden `drop` + `product` prefill | `apparel-drop-list` | `shop-apparel` | Controlled snippets now; first access when approved details and the eventual drop are real; no payment implied | Low; the page does not collect body measurements, and names/emails never ride on the analytics event layer |
