...
| Source of truth | `03_PRESSURE_PLANNER_APP/` (governed product repo) | current | Canonical product source for Pressure Planner runtime, QA, Android-init work, store asset prep, and future app builds. **Not this repo.** |
...
1. The Pressure Planner application source remains governed in `03_PRESSURE_PLANNER_APP/`.
2. That source deploys to its own infrastructure (separate Vercel project, Render, Railway, or continued Manus, whichever survives the dependency audit on MySQL / S3 / OAuth / Manus runtime).
